 ...
 * #DOC
 * #DOC 	SSet_U32: 32-bit unsigned int key
 * #DOC
 * #DOC 	SSet_I: 64-bit int key
 * #DOC
 * #DOC 	SSet_S: string key
 * #DOC
 *
...
/* #API: |Allocate set (heap)|set type; initial reserve|set|O(1)|0;1| */
...
/* #API: |Duplicate set|input set|output set|O(n)|0;1| */
S_INLINE sms_t *sms_dup(const sms_t *src)
{
	return sm_dup(src);
}

/* #API: |Reset/clean set (keeping set type)|set|-|O(1) for simple sets, O(n) for sets having nodes with strings|0;1| */
...
/*
#API: |Free one or more sets (heap)|set; more sets (optional)|-|O(1) for simple sets, O(n) for string sets|0;1|
...
/*
#API: |Ensure space for extra elements|set;number of extra elements|extra size allocated|O(1)|0;1|
size_t sms_grow(sms_t **s, const size_t extra_elems)

#API: |Ensure space for elements|set;absolute element reserve|reserved elements|O(1)|0;1|
size_t sms_reserve(sms_t **s, const size_t max_elems)

#API: |Make the set use the minimum possible memory|set|set reference (optional usage)|O(1) for allocators using memory reset; O(n) for naive allocators|0;1|
sms_t *sms_shrink(sms_t **s);

#API: |Get set size|set|Set number of elements|O(1)|0;1|
size_t sms_size(const sms_t *s);

#API: |Allocated space|set|current allocated space (vector elements)|O(1)|0;1|
size_t sms_capacity(const sms_t *s);

#API: |Preallocated space left|set|allocated space left|O(1)|0;1|
size_t sms_capacity_left(const sms_t *s);

#API: |Tells if a set is empty (zero elements)|set|S_TRUE: empty vector; S_FALSE: not empty|O(1)|0;1|
...
/* #API: |Set element count/check|set; integer key|S_TRUE: element found; S_FALSE: not in the set|O(log n)|0;1| */
S_INLINE sbool_t sms_count_i(const sms_t *s, const int64_t k)
{
	return sm_count_i(s, k);
}

/* #API: |Set element count/check|set; string key|S_TRUE: element found; S_FALSE: not in the set|O(log n)|0;1| */
...
/* #API: |Enumerate set elements in a given key range|set; key lower bound; key upper bound; callback function; callback function context|Elements processed|O(log n) + O(log m); additional 2 * O(log n) space required, allocated on the stack, i.e. fast|0;1| */
S_INLINE size_t sms_itr_i(const sms_t *s, int64_t key_min, int64_t key_max, sm_it_i_t f, void *context)
{
	return sm_itr_i(s, key_min, key_max, f, context);
}

/* #API: |Enumerate set elements in a given key range|set; key lower bound; key upper bound; callback function; callback function context|Elements processed|O(log n) + O(log m); additional 2 * O(log n) space required, allocated on the stack, i.e. fast|0;1| */
S_INLINE size_t sms_itr_s(const sms_t *s, const ss_t *key_min, const ss_t *key_max, sm_it_s_t f, void *context)
{
	return sm_itr_s(s, key_min, key_max, f, context);
}
