...
/* #API: |Allocate distributed map (heap)|map type; number of submaps; initial reserve|map|O(1)|1;2| */
sdm_t *sdm_alloc(const enum eSM_Type t, const size_t number_of_submaps, const size_t initial_num_elems_reserve);

/* #API: |Make the dynamic map use the minimum possible memory|map|-|O(1) for allocators using memory remap; O(n) for naive allocators|0;2| */
void sdm_shrink(sdm_t **s);

/* #API: |Duplicate distributed map|input map|output map|O(n)|0;2| */
sdm_t *sdm_dup(const sdm_t *src);

/* #API: |Reset/clean distributed map (keeping map type)|map|S_TRUE: OK, S_FALSE: invalid map|O(1) for simple maps, O(n) for maps having nodes with strings|0;2| */
...
/* #API: |Set routing functions|map; routing function for integer keys (if NULL, a "fair" hash will be used); routing function for string keys (if NULL, also a "fair" hash will be used)|-|O(1)|0;2| */
...
/* #API: |Get route to subtree (string key)|dmap; key|subtree id|O(1)|0;2| */
...
/* #API: |Get number of submaps|dmap|Number of submaps|O(1)|1;2| */
size_t sdm_size(const sdm_t *m);

/* #API: |Get submap pointer vector|dmap|map vector|O(1)|1;2| */
sm_t **sdm_submaps(sdm_t *m);

/* #API: |Get submap pointer vector (read-only)|dmap|map vector|O(1)|0;2| */
