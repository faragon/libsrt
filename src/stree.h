...
/* #API: |Allocate tree (heap)|tree configuration;space preallocated to store n elements|allocated tree|O(1)|1;1| */
st_t *st_alloc(const struct STConf *f, const size_t initial_reserve);

SD_BUILDPROTS(st)

/*
#API: |Free one or more trees (heap)|tree;more trees (optional)|-|O(1)|1;1|
void st_free(st_t **c, ...)
...
#API: |Equivalent to st_size|tree|number of tree nodes|O(1)|1;1|
...
/* #API: |Insert element into tree|tree; element to insert|S_TRUE: OK, S_FALSE: error (not enough memory)|O(log n)|1;1| */
sbool_t st_insert(st_t **t, const stn_t *n);

/* #API: |Delete tree element|tree; element to delete; node delete handling callback (optional if e.g. nodes use no extra dynamic memory references)|S_TRUE: found and deleted; S_FALSE: not found|O(log n)|1;1| */
sbool_t st_delete(st_t *t, const stn_t *n, stn_callback_t callback);

/* #API: |Locate node|tree; node|Reference to the located node; NULL if not found|O(log n)|1;1| */
...
/* #API: |Full tree traversal: pre-order|tree; traverse callback; callback context|Number of levels stepped down|O(n)|1;1| */
ssize_t st_traverse_preorder(const st_t *t, st_traverse f, void *context);

/* #API: |Full tree traversal: in-order|tree; traverse callback; callback context|Number of levels stepped down|O(n)|1;1| */
ssize_t st_traverse_inorder(const st_t *t, st_traverse f, void *context);

/* #API: |Full tree traversal: post-order|tree; traverse callback; callback context|Number of levels stepped down|O(n)|1;1| */
ssize_t st_traverse_postorder(const st_t *t, st_traverse f, void *context);

/* #API: |Bread-first tree traversal|tree; traverse callback; callback contest|Number of levels stepped down|O(n); Aux space: n/2 * sizeof(stndx_t)|1;1| */
ssize_t st_traverse_levelorder(const st_t *t, st_traverse f, void *context);

/* #API: |Tree check (debug purposes)|tree|S_TREE: OK, S_FALSE: breaks RB tree rules|O(n)|1;1| */
